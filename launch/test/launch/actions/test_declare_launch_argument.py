...
def test_declare_launch_argument_methods():
    ...
    assert dla1.arg_choices is None
    ...
    assert dla2.arg_choices is None
    assert dla2.description, 'description does not have a non-empty default value'

    dla3 = DeclareLaunchArgument('name', description='description', arg_choices=["var1", "var2"])
    assert dla3.default_value is None
    assert dla3.arg_choices == ["var1", "var2"]
    assert str(dla3.arg_choices) in dla3.description
...
def test_declare_launch_argument_execute():
    ...
    action3 = DeclareLaunchArgument('name', default_value="var1", arg_choices=["var1", "var2"])
    lc3 = LaunchContext()
